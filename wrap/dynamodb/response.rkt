...
(define (parse-item-value type json)    
  (if (hash? json)
      (let*: ((json : JsObject (cast json JsObject))
              (value : Json (hash-ref (cast json JsObject) type)))
        (if (or (string? value)
                (and (list? value)
                     (andmap string? value)))
            value
    ...
    (let: loop : (HashTable String Item) 
      ...
      (if (null? attrs)
          ...
          (let* ((jitem (car attrs))
            ...
            (if (hash? type-value)
                (begin
                  (hash-set! items name (parse-item name (cast type-value JsObject)))
                  (loop (cdr attrs)))
                (parse-fail jattrs)))))))
...
(define (parse-last-key resp)
  ...
  (define (item-val resp key)
    (if (hash-has-key? resp key)
        (let ((key (hash-ref resp key)))
          (if (hash? key)
              (parse-keyval (cast key JsObject))
              #f))
        #f))
  ...
  (if (hash-has-key? resp 'LastEvaluatedKey)
      (let ((attrs (hash-ref resp 'LastEvaluatedKey)))
        (if (hash? attrs)
            (let* ((attrs (cast attrs JsObject))
                   (hash-key (item-val attrs 'HashKeyElement))
